...
class QuerysetLookups(object):
    ...
    def __init__(self, model: Type[models.Model], model_pool: List[Type[models.Model]],
                 lookup_tree: LookupTree, parent: 'QuerysetLookups'=None, prefixes: List[str] = None,
                 visited_models=None):
        self.model = model
    ...
    def _field_type_declarations(self, field: models.Field):
        ...
        if isinstance(field, (models.ForeignKey, models.OneToOneField, models.ManyToOneRel, models.ManyToManyField)):
            if (field.related_model in self.model_pool) and (field.related_model not in self.visited_models) \
                    and (field.related_model in self.lookup_tree):
                child_queryset_lookups = QuerysetLookups(model=field.related_model,
                                                         ...
                                                         lookup_tree=self.lookup_tree.sub_tree(root_model=field.related_model))
